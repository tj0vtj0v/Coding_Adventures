...
class Manager():
    def __init__(self, obj_class):
        self.repository = [[set() for _ in range(SPATIAL_PARTITIONING_COLS)] for _ in
                           range(SPATIAL_PARTITIONING_ROWS)]  # list of sets
        ...
        while q:
            ...
            if self.checkOverlap(sq_radius, x, y, row, col):
                ...
                for mod in modifications:
                    ...
                    if new_tile not in checked and new_tile not in q and new_tile[0] >= 0 and new_tile[
                        0] < SPATIAL_PARTITIONING_ROWS and new_tile[1] >= 0 and new_tile[
                        1] < SPATIAL_PARTITIONING_COLS and self.repository[new_tile[0]][new_tile[1]]:
                        q.append(new_tile)
        return output
